...
export const MyTasks: React.FC = () => {
  ...
  const filteredTasks = React.useMemo(() => {
    ...
    if (sortBy === 'dueDate') {
      ...
      filtered = filtered.sort((a, b) => {
        const aProject = a.taskGroupId?.projectId?.name || 'Unknown';
        const bProject = b.taskGroupId?.projectId?.name || 'Unknown';
  ...
  const groupedTasks = React.useMemo(() => {
    if (sortBy === 'project') {
      ...
      filteredTasks.forEach(task => {
        const projectId = task.taskGroupId?.projectId?._id || 'unknown';
        if (!groups[projectId]) {
          groups[projectId] = [];
        }
      ...
      return Object.entries(groups).map(([projectId, tasks]) => ({
        id: projectId,
        name: tasks[0].taskGroupId?.projectId?.name || 'Unknown Project',
        tasks,
      }));
    } else {
      return [{
                  ...
                  >
                    ...
                    <div className="col-span-2">
                      <span className="text-sm text-[var(--text-primary)] bg-[var(--bg-primary)] px-2 py-1 rounded">
                        {task.taskGroupId?.projectId?.name || 'Unknown Project'}
                      </span>
                    </div>
